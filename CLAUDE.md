...
- **One live gradient renderer exists: `AlbumGradientMesh.jsx`** (imported as `GradientBackground` in `LiveScreen.jsx` — the file `GradientBackground.jsx` also exists in the repo with its own test suite but is NOT imported anywhere live; don't confuse the two, don't edit the wrong one). Do not add engine A/B switches, URL/localStorage renderer selection, or parallel experimental renderers. Debug palette input first; renderer tuning cannot repair bad extracted colors.
