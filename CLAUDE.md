...
- **One live gradient renderer exists.** `GradientBackground.jsx` is the palette-driven Canvas2D two-light renderer. Do not add engine A/B switches, URL/localStorage renderer selection, or parallel experimental renderers. Debug palette input first; renderer tuning cannot repair bad extracted colors.
