...
describe('palette client cache version', () => {
  it('rotates cache keys for the v6 accent index/weight response', () => {
    expect(usePaletteModule.PALETTE_VERSION).toBe(6)
  })
})
...
describe('buildWeights', () => {
  ...
  it('gives a synthetic (population=null) entry a fixed weight, real entries split the remainder proportionally', () => {
    const w = buildWeights([{ population: 900 }, { population: 100 }, { population: null }])
    // synthetic gets ACCENT_WEIGHT (0.30 -- 2026-08-03, was 0.15: at 0.15 the
    // accent's two-light render collapsed to an isolated circle instead of
    // a shared field), the two real entries split the remaining 0.70 in
    // their 900:100 (9:1) ratio
    expect(w[2]).toBeCloseTo(0.30, 5)
    expect(w[0]).toBeCloseTo(0.70 * 0.9, 5)
    expect(w[1]).toBeCloseTo(0.70 * 0.1, 5)
