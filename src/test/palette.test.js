...
describe('buildWeights', () => {
  ...
  it('gives a synthetic (population=null) entry a fixed weight, real entries split the remainder proportionally', () => {
    ...
    // synthetic gets ACCENT_WEIGHT (0.40 -- 2026-08-03, was 0.15 then 0.30:
    // owner asked for 60/40 generally after live-checking several covers),
    // the two real entries split the remaining 0.60 in their 900:100 (9:1)
    // ratio
    expect(w[2]).toBeCloseTo(0.40, 5)
    expect(w[0]).toBeCloseTo(0.60 * 0.9, 5)
    expect(w[1]).toBeCloseTo(0.60 * 0.1, 5)
